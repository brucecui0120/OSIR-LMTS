...
class StatisticAll:
    ...
    def process_data(self):
        ...
        new_result_data = {}
        # 存储成{公司名：{语言模态数据集数量: , 语言模态下载量: , 语音模态数据集数量: , 语音模态下载量: , 视觉模态数据集数量: , 视觉模态下载量: , 多模态模态数据集数量: , 多模态模态下载量: , 具身模态数据集数量: , 具身模态下载量: }}
        for company_name in self.company_name_list:
            ...
            for modality_name in self.modality_name_list:
                ...
                modality_data[modality_name + '模态数据集数量'] = modality_count[0] if len(modality_count) > 0 else 0
                modality_data[modality_name + '模态下载量'] = modality_download[0] if len(modality_download) > 0 else 0
            new_result_data[company_name] = modality_data
        # 存储到新的excel表格 按照 语言数据集个数 语音数据集个数 视觉数据集个数 多模态数据集个数 具身数据集个数 语言数据集下载量 语音数据集下载量 视觉数据集下载量 多模态数据集下载量 具身数据集下载量
        ...
        for company_name in self.company_name_list:
            ...
            for life_cycle in self.life_cycle_list:
                ...
                life_cycle_data[life_cycle + '生命周期数据集数量'] = life_cycle_count[0] if len(
                    life_cycle_count) > 0 else 0
                life_cycle_data[life_cycle + '生命周期下载量'] = life_cycle_download[0] if len(
                    life_cycle_download) > 0 else 0
...
def combine_data():
    ...
    if current_month == 1:
        current_month = 13
    last_month_path = f'./data_each_month/month{current_month - 1}_processed(anno_benchmark_embodied).xlsx'
...
if __name__ == '__main__':
    ...
    get_final_data()
    # 统计数据
    statistic = StatisticAll(r'D:\Workspace\ProgramWorkspace\Python\GithubProject\data-collector-update\crawl_data\current_month_corrected.xlsx')
